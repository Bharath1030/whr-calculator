...
const JOB_IMPACT_BY_OFFTAKE: Record<Offtake, string> = {
  dac: "~20-40 direct roles (plant ops, maintenance)",
  waterTreatmentFO: "~15-30 direct roles (treatment ops)",
  atmosphericWater: "~8-20 direct roles (capture ops)",
  districtHeat: "~5-15 direct roles (network ops)",
  hotWater: "~3-10 direct roles (system ops)",
  greenhouses: "~200 direct roles per 30 ha; 2-3x indirect",
  foodBrewery: "~30-60 direct roles (process ops)",
};
...
export default function Page() {
  ...
  const [enableGigawattScale, setEnableGigawattScale] = useState(false);
  const [itLoadMW, setItLoadMW] = useState(100);
  ...
  const normalized = useMemo(() => {
    ...
    const greenhouseTons_per_MWyr = greenhouseHa_per_MWyr * TOMATO_TONS_PER_HA_YEAR;
  ...
  const URAVU_GROSS_M3_PER_MWYR_RANGE = { min: 6247.29, max: 14038.46 };
  const URAVU_NET_M3_PER_MWYR_RANGE = { min: 3205.9, max: 12391.9 };
  const URAVU_NVW_GROSS_RANGE = { min: 8.96, max: 16.72 };
  const URAVU_NVW_NET_RANGE = { min: 11.06, max: 25.61 };
  const URAVU_NVW_CREDIT_RANGE = { min: -24.17, max: 3.18 };
  ...
  const FAQ_OFFTAKE_SECTIONS: Record<Offtake, FaqSection[]> = {
    ...
    atmosphericWater: [
      {
        title: "How does atmospheric water capture work?",
        items: [
          {
            q: "What system is modeled for atmospheric water capture?",
            a: "The FAQ reflects a waste-heat-driven capture system with a dry cooler and air-cooled chiller.",
          },
          {
            q: "What operating temperatures are assumed?",
            a: "The model uses 35°C to 45°C return water and 23°C to 33°C supply water in the modeled cases.",
          },
          {
            q: "What design capacity is assumed?",
            a: "The reference design uses a 10 MW system, with results normalized per MW-year for comparison.",
          },
        ],
      },
      {
        title: "What water output and economics should I expect?",
        items: [
          {
            q: "What water output range is expected per MW-year?",
            a: `Gross production ranges about ${fmt2(URAVU_GROSS_M3_PER_MWYR_RANGE.min)}–${fmt2(URAVU_GROSS_M3_PER_MWYR_RANGE.max)} m3/MW-year, with net production around ${fmt2(URAVU_NET_M3_PER_MWYR_RANGE.min)}–${fmt2(URAVU_NET_M3_PER_MWYR_RANGE.max)} m3/MW-year depending on climate and temperature.`,
          },
          {
            q: "What is the net value of water (NVW) range?",
            a: `Modeled NVW (gross) spans about $${fmt2(URAVU_NVW_GROSS_RANGE.min)}–$${fmt2(URAVU_NVW_GROSS_RANGE.max)} per m3, and NVW (net) spans $${fmt2(URAVU_NVW_NET_RANGE.min)}–$${fmt2(URAVU_NVW_NET_RANGE.max)} per m3 before cooling credit.`,
          },
          {
            q: "How does cooling credit affect NVW?",
            a: `With cooling credit, NVW ranges roughly $${fmt2(URAVU_NVW_CREDIT_RANGE.min)} to $${fmt2(URAVU_NVW_CREDIT_RANGE.max)} per m3, indicating strong dependence on local electricity and cooling economics.`,
          },
        ],
      },
      {
        title: "What are the main cost and operational drivers?",
        items: [
          {
            q: "What CapEx components are included?",
            a: "The modeled configuration includes an Uravu module, dry cooler, and air-cooled chiller with total CapEx near $1.0M for the reference unit.",
          },
          {
            q: "What electricity costs are used?",
            a: "The workbook uses $0.095–$0.20 per kWh depending on region (Bengaluru to Frankfurt cases).",
          },
          {
            q: "What maintenance assumptions are used?",
            a: "Maintenance rates are modeled at 2% for Uravu and dry cooler, and 3% for the air-cooled chiller, with a 15-year amortization period.",
          },
        ],
      },
    ],
    ...
    greenhouses: [
      {
        title: "How does greenhouse heat reuse work?",
        items: [
          {
            q: "What greenhouse use case is modeled here?",
            a: "A standardized 30-hectare high-tech Venlo greenhouse for high-wire tomato cultivation is used as the reference case.",
          },
          {
            q: "What thermal load is assumed for the greenhouse campus?",
            a: "Peak thermal load is modeled at about 10 MW to maintain a 30°C delta against winter ambient conditions.",
          },
          {
            q: "How does the WHR integration interface work?",
            a: "The data center loop is separated from the greenhouse loop with a plate heat exchanger, and heat is upgraded using water-to-water heat pumps.",
          },
          {
            q: "What temperature range is available from the data center?",
            a: "The reference case assumes 30–45°C return water from the data center, upgraded to 45–60°C for greenhouse use.",
          },
          {
            q: "How is seasonal mismatch handled?",
            a: "The model uses a cascaded approach with HT-ATES thermal storage so summer surplus can be used in winter peaks.",
          },
        ],
      },
      {
        title: "What are the cost, payback, and efficiency impacts?",
        items: [
          {
            q: "Which energy configuration is most attractive?",
            a: "Scenario D (WHR + geothermal + ATES) is the preferred configuration, reducing thermal energy costs by up to 78% versus the gas baseline.",
          },
          {
            q: "What heat price makes WHR competitive?",
            a: "A nominal heat price around $0.01/kWh keeps WHR below gas parity and helps offset the cost of external CO2.",
          },
          {
            q: "What is the expected payback for the greenhouse operator?",
            a: "Modeled payback for the WHR investment is about 0.1 to 2.1 years when sized to cover 60–80% of annual heat load.",
          },
          {
            q: "How do heat pumps perform in the WHR scenario?",
            a: "Because the source temperature is high (30°C+), water-to-water heat pumps are modeled at COP 5.0–6.0.",
          },
          {
            q: "What are the key CAPEX ranges?",
            a: "The data center side is estimated at about $3.0M, and the greenhouse side at $1.0M-$4.0M depending on required lift.",
          },
        ],
      },
      {
        title: "What DC performance benefits and risks should I expect?",
        items: [
          {
            q: "How does this help meet ERF compliance?",
            a: "In Germany, EnEfG requires ERF of 10% by 2026 and 20% by 2028; a greenhouse offers a reliable large-scale heat sink.",
          },
          {
            q: "What are the site drivers in Frankfurt and Middenmeer?",
            a: "Frankfurt is compliance-driven (EnEfG). Middenmeer is economics-driven with grid congestion and existing heat networks.",
          },
          {
            q: "What CO2 impacts are modeled?",
            a: "A 30-hectare WHR greenhouse reduces emissions to ~1,500 tCO2/yr versus ~18,000 tCO2/yr baseline, assuming grid mix.",
          },
          {
            q: "What water benefits are expected?",
            a: "Closed-loop heat export reduces DC evaporative losses, and the greenhouse can harvest ~240,000 m3/yr rainwater.",
          },
          {
            q: "What is the main operational risk?",
            a: "Interdependency risk is mitigated by thermal storage (HT-ATES) that buffers maintenance or demand mismatch.",
          },
        ],
      },
      {
        title: "What is the socio-economic impact?",
        items: [
          {
            q: "What job creation impact is modeled?",
            a: "A 30-hectare campus supports roughly 200 direct jobs, with 2–3 indirect jobs per direct job.",
          },
          {
            q: "What food security benefit is expected?",
            a: "The campus can produce about 13,500 tons of tomatoes annually, serving roughly 2.5 million people.",
          },
          {
            q: "Why does this help community acceptance?",
            a: "The greenhouse reframes the data center as a community asset by producing local food and reducing emissions.",
          },
        ],
      },
    ],
    ...
    <main className="mx-auto max-w-5xl p-6 font-sans">
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-2">
        {[
          ...
          { id: "whr-regulatory", label: "Regulatory" },
              ...
              {inputTab === "dc" ? (
                <>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="text-xs font-semibold text-slate-700">IT load scale</div>
                    <label className="flex items-center gap-2 text-xs text-slate-700">
                      <input
                        type="checkbox"
                        checked={enableGigawattScale}
                        onChange={(e) => {
                          const next = e.target.checked;
                          setEnableGigawattScale(next);
                          if (!next && itLoadMW > 100) setItLoadMW(100);
                        }}
                        className="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-200"
                      />
                      Future scale (up to 1 GW)
                    </label>
                  </div>
                  <SliderRow
                    label="IT load"
                    value={itLoadMW}
                    setValue={setItLoadMW}
                    min={0}
                    max={enableGigawattScale ? 1000 : 100}
                    step={0.5}
                    unit="MW"
                    decimals={1}
        ...
        >
          <div className="space-y-4">
            <div className="mb-4">
              ...
              <div className="mt-2 text-xs text-slate-600">
                Scaling: outputs are based on recoverable heat of {fmt1(core.recoverableHeatMW)} MW. For a 10 MW
                system, set recoverable heat to 10 MW.
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                <div className="text-sm font-semibold text-emerald-900 mb-1">
                  Job creation impact (selected offtake)
                </div>
                <div className="text-sm text-emerald-900">
                  {OFFTAKE_LABEL[offtake]}: {JOB_IMPACT_BY_OFFTAKE[offtake]}
                </div>
                <div className="text-xs text-emerald-800 mt-1">
                  Indicative direct jobs; scale varies by site, throughput, and operating model.
                </div>
              </div>
            ...
            <div className="rounded-xl border border-orange-200 bg-gradient-to-br from-orange-50 to-amber-50 p-4 min-w-0">
              ...
              <div className="mt-2 space-y-2">
                ...
                <MetricRow
                  ...
                  value={`${fmt2(normalized.greenhouseHa_per_MWyr)} ha`}
                  sub={`~${fmtInt(normalized.greenhouseTons_per_MWyr)} tons tomatoes/yr`}
                />
              </div>
      ...
      <div className="mt-8" id="whr-regulatory">
        ...
        >
          ...
          <div
            ...
            tabIndex={0}
            onKeyDown={(e) => {
              if (e.key === "Enter" || e.key === " ") setShowRegulation(true);
            }}
          >
            <div>
              <div className="text-sm font-semibold text-slate-900">Why this matters</div>
              <p className="mt-2 text-sm text-slate-700">
                EMEA waste-heat reuse mandates drive permitting timelines, compliance risk, and revenue impact.
                Click to view the regulation map and key milestones.
              </p>
              <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-4 py-2 text-xs font-semibold text-slate-800 shadow-sm">
                View regulation map
              </div>
            </div>
            <div className="flex items-center justify-center">
              <img
                src="/Images/WHR%20mandate.jpg"
                alt="Waste Heat Reuse mandates in EMEA"
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="hidden sm:flex flex-wrap gap-2">
                {faqOfftakeOptions.map((key) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-1.5 text-sm font-semibold border shadow-sm transition-colors ${
                      faqOfftake === key
              ...
              <select
                className="sm:hidden w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-900"
                value={faqOfftake}
            ...
            <div className="space-y-4">
              {[...FAQ_GENERAL_SECTIONS, ...(FAQ_OFFTAKE_SECTIONS[faqOfftake] || [])].map(
                (section, idx) => (
                  ...
                  >
                    ...
                    {section.title === "Socio-economic impact" ? (
                      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
                        <div className="text-sm font-semibold text-slate-900 mb-2">
                          Job market and new job creation (indicative)
                        </div>
                        <div className="w-full overflow-x-auto">
                          <table className="min-w-full text-sm">
                            <thead>
                              <tr className="text-left text-xs text-slate-500">
                                <th className="py-2 pr-4">Offtake</th>
                                <th className="py-2">Indicative direct jobs</th>
                              </tr>
                            </thead>
                            <tbody className="text-slate-700">
                              <tr className="border-t border-slate-200">
                                <td className="py-2 pr-4">Direct Air Capture (DAC)</td>
                                <td className="py-2">{JOB_IMPACT_BY_OFFTAKE.dac}</td>
                              </tr>
                              <tr className="border-t border-slate-200">
                                <td className="py-2 pr-4">Waste Water Treatment (FO)</td>
                                <td className="py-2">{JOB_IMPACT_BY_OFFTAKE.waterTreatmentFO}</td>
                              </tr>
                              <tr className="border-t border-slate-200">
                                <td className="py-2 pr-4">Atmospheric Water Capture</td>
                                <td className="py-2">{JOB_IMPACT_BY_OFFTAKE.atmosphericWater}</td>
                              </tr>
                              <tr className="border-t border-slate-200">
                                <td className="py-2 pr-4">District Heating</td>
                                <td className="py-2">{JOB_IMPACT_BY_OFFTAKE.districtHeat}</td>
                              </tr>
                              <tr className="border-t border-slate-200">
                                <td className="py-2 pr-4">Hot Water</td>
                                <td className="py-2">{JOB_IMPACT_BY_OFFTAKE.hotWater}</td>
                              </tr>
                              <tr className="border-t border-slate-200">
                                <td className="py-2 pr-4">Greenhouses</td>
                                <td className="py-2">{JOB_IMPACT_BY_OFFTAKE.greenhouses}</td>
                              </tr>
                              <tr className="border-t border-slate-200">
                                <td className="py-2 pr-4">Food & Brewery</td>
                                <td className="py-2">{JOB_IMPACT_BY_OFFTAKE.foodBrewery}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                        <div className="mt-2 text-xs text-slate-500">
                          Indicative ranges vary by site, scale, and operating model.
                        </div>
                      </div>
                    ) : null}
                  </details>
              ...
              {faqOfftake === "waterTreatmentFO" ? (
                <details className="rounded-xl border border-slate-200 bg-white p-4" open>
                  <summary className="cursor-pointer text-sm font-semibold text-slate-900">
                    System Performance Metrics / Cheat Sheet
                  </summary>
                        ...
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              ...
                              <td className="py-2 font-semibold text-slate-900">${OFFTAKE_PLANT_COSTS.waterTreatmentFO.capexM.toFixed(2)}M</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">OpEx (annual)</td>
                              <td className="py-2 font-semibold text-slate-900">${fmtInt(OFFTAKE_PLANT_COSTS.waterTreatmentFO.opexPerYear)}</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">LCOW range</td>
                              <td className="py-2 font-semibold text-slate-900">$3.29-$3.68 per m3</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                            ...
                            <tr className="border-t border-slate-200">
                              ...
                              <td className="py-2 font-semibold text-slate-900">{fmtInt(treviThroughputMidM3)} m3/yr</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Temp performance factor</td>
                              <td className="py-2 font-semibold text-slate-900">{fmt2(perf.treviFactor)}x (from return temp)</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                        ...
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              ...
                              <td className="py-2 font-semibold text-slate-900">{dcConfig}</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">PUE baseline</td>
                              <td className="py-2 font-semibold text-slate-900">{fmtOrDash(dcSavings.pueBaseline, 2)}</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">PUE with WHR</td>
                              <td className="py-2 font-semibold text-slate-900">{fmtOrDash(dcSavings.pueWithHR, 2)}</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">PUE reduction</td>
                              <td className="py-2 font-semibold text-slate-900">{fmtOrDash(dcSavings.pueReduction, 1)}%</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">WUE baseline</td>
                              <td className="py-2 font-semibold text-slate-900">{fmtOrDash(dcSavings.wueBaseline, 3)}</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">WUE with WHR</td>
                              <td className="py-2 font-semibold text-slate-900">{fmtOrDash(dcSavings.wueWithHR, 3)}</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">WUE reduction</td>
                              <td className="py-2 font-semibold text-slate-900">{fmtOrDash(dcSavings.wueReduction, 1)}%</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">ERF</td>
                              <td className="py-2 font-semibold text-slate-900">{fmt1(dcSavings.erf * 100)}%</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">ERE (heuristic)</td>
                              <td className="py-2 font-semibold text-slate-900">{fmtOrDash(dcSavings.ere, 2)}</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                        ...
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              ...
                              <td className="py-2 font-semibold text-slate-900">{fmtInt(normalized.treviM3_per_MWyr)} m3</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">People served (Phoenix equivalent)</td>
                              <td className="py-2 font-semibold text-slate-900">{fmt1(normalized.phxPeoplePerMWyr_trevi)}</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">DAC water byproduct</td>
                              <td className="py-2 font-semibold text-slate-900">{fmtInt(normalized.dacWaterM3_per_MWyr)} m3</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
              ...
              {faqOfftake === "atmosphericWater" ? (
                <details className="rounded-xl border border-slate-200 bg-white p-4" open>
                  ...
                  <div className="mt-4 space-y-6">
                    <div>
                      ...
                      <div className="w-full overflow-x-auto">
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Capture module CapEx</td>
                              <td className="py-2 font-semibold text-slate-900">$0.50M</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Dry cooler CapEx</td>
                              <td className="py-2 font-semibold text-slate-900">$0.15M</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Air-cooled chiller CapEx</td>
                              <td className="py-2 font-semibold text-slate-900">$0.35M</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Total reference CapEx</td>
                              <td className="py-2 font-semibold text-slate-900">$1.00M</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Electricity cost range</td>
                              <td className="py-2 font-semibold text-slate-900">$0.095-$0.20/kWh</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                    ...
                    <div>
                      <div className="text-xs font-semibold text-slate-800 mb-2">
                        Water production ranges (per MW-year)
                      </div>
                      <div className="w-full overflow-x-auto">
                        <table className="min-w-full text-sm">
                          <thead>
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Gross production</td>
                              <td className="py-2 font-semibold text-slate-900">
                                {fmt2(URAVU_GROSS_M3_PER_MWYR_RANGE.min)}-{fmt2(URAVU_GROSS_M3_PER_MWYR_RANGE.max)} m3
                              </td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Net production</td>
                              <td className="py-2 font-semibold text-slate-900">
                                {fmt2(URAVU_NET_M3_PER_MWYR_RANGE.min)}-{fmt2(URAVU_NET_M3_PER_MWYR_RANGE.max)} m3
                              </td>
                            </tr>
                          </tbody>
                        </table>
                    ...
                    <div>
                      ...
                      <div className="w-full overflow-x-auto">
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">NVW (gross)</td>
                              <td className="py-2 font-semibold text-slate-900">
                                ${fmt2(URAVU_NVW_GROSS_RANGE.min)}-${fmt2(URAVU_NVW_GROSS_RANGE.max)} per m3
                              </td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">NVW (net)</td>
                              <td className="py-2 font-semibold text-slate-900">
                                ${fmt2(URAVU_NVW_NET_RANGE.min)}-${fmt2(URAVU_NVW_NET_RANGE.max)} per m3
                              </td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">NVW (with cooling credit)</td>
                              <td className="py-2 font-semibold text-slate-900">
                                ${fmt2(URAVU_NVW_CREDIT_RANGE.min)}-${fmt2(URAVU_NVW_CREDIT_RANGE.max)} per m3
                              </td>
                            </tr>
                          </tbody>
                        </table>
              ...
              {faqOfftake === "greenhouses" ? (
                <details className="rounded-xl border border-slate-200 bg-white p-4" open>
                  ...
                  <div className="mt-4 space-y-6">
                    <div>
                      ...
                      <div className="w-full overflow-x-auto">
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Greenhouse size</td>
                              <td className="py-2 font-semibold text-slate-900">30 ha (Venlo-style, tomatoes)</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Peak thermal load</td>
                              <td className="py-2 font-semibold text-slate-900">~10 MW (Delta T 30C)</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">DC return temperature</td>
                              <td className="py-2 font-semibold text-slate-900">30-45C</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Heat pump COP</td>
                              <td className="py-2 font-semibold text-slate-900">5.0-6.0 (water-to-water)</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Target heat coverage</td>
                              <td className="py-2 font-semibold text-slate-900">60-80% of annual load</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Greenhouse area per MW-year</td>
                              <td className="py-2 font-semibold text-slate-900">{fmt2(normalized.greenhouseHa_per_MWyr)} ha</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Tomato output per MW-year</td>
                              <td className="py-2 font-semibold text-slate-900">~{fmtInt(normalized.greenhouseTons_per_MWyr)} tons/yr</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                    ...
                    <div>
                      <div className="text-xs font-semibold text-slate-800 mb-2">
                        Economic summary (Scenario D - WHR)
                      </div>
                      <div className="w-full overflow-x-auto">
                        <table className="min-w-full text-sm">
                          <thead>
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Thermal cost reduction</td>
                              <td className="py-2 font-semibold text-slate-900">Up to 78% vs gas baseline</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Heat sales price</td>
                              <td className="py-2 font-semibold text-slate-900">~$0.01/kWh (thermal)</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Effective heat cost</td>
                              <td className="py-2 font-semibold text-slate-900">$0.012-$0.02/kWh_th</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">WHR payback</td>
                              <td className="py-2 font-semibold text-slate-900">0.1-2.1 years</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">WHR CAPEX range</td>
                              <td className="py-2">$4M-$8M (heat pumps/interconnects)</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="w-full overflow-x-auto">
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">CO2 emissions (baseline)</td>
                              <td className="py-2">~18,000 tCO2/yr</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">CO2 emissions (WHR)</td>
                              <td className="py-2">~1,500 tCO2/yr</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Gas displaced</td>
                              <td className="py-2">~10.5 million m3/yr</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Rainwater harvested</td>
                              <td className="py-2">~240,000 m3/yr</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Annual tomato output</td>
                              <td className="py-2">~13,500 tons</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">People supported</td>
                              <td className="py-2">~2.5 million (5.5 kg/person/yr)</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Direct jobs</td>
                        ...
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              ...
                              <td className="py-2 font-semibold text-slate-900">8-10 MW per facility</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Heat share of site energy</td>
                              <td className="py-2 font-semibold text-slate-900">~2/3 of total site energy</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Compatible process range</td>
                              <td className="py-2 font-semibold text-slate-900">~60-80°C (cleaning, process heat)</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Pasteurization range</td>
                              <td className="py-2 font-semibold text-slate-900">~90°C</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">DC output scenarios</td>
                              <td className="py-2 font-semibold text-slate-900">30°C baseline, 65°C future</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Target return temperature</td>
                              <td className="py-2 font-semibold text-slate-900">18°C or lower</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                        ...
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              ...
                              <td className="py-2 font-semibold text-slate-900">~3.3</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">8 MW delivered heat</td>
                              <td className="py-2 font-semibold text-slate-900">5.6 MW DC heat + 2.4 MW electricity</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Proximity (EU DCs)</td>
                              <td className="py-2 font-semibold text-slate-900">~50% within 3 km, ~23% within 1 km</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                        ...
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="text-slate-700">
                            <tr className="border-t border-slate-200">
                              ...
                              <td className="py-2 font-semibold text-slate-900">${OFFTAKE_PLANT_COSTS.foodBrewery.capexM.toFixed(2)}M</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">OpEx (annual)</td>
                              <td className="py-2 font-semibold text-slate-900">${fmtInt(OFFTAKE_PLANT_COSTS.foodBrewery.opexPerYear)}</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Offtaker payback range</td>
                              <td className="py-2 font-semibold text-slate-900">~2-5 years</td>
                            </tr>
                            <tr className="border-t border-slate-200">
                              <td className="py-2 pr-4">Data center payback</td>
                              <td className="py-2 font-semibold text-slate-900">&lt; 1 year (case study range)</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
      ...
      {showRegulation ? (
        ...
        >
          <div
            className="relative max-w-5xl w-full rounded-2xl bg-white p-3 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              className="absolute right-3 top-3 rounded-full border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700 shadow-sm"
              onClick={() => setShowRegulation(false)}
              aria-label="Close regulation map"
            >
              Close
            </button>
            <img
      ...
      <button
        ...
        className="fixed bottom-6 right-6 h-12 w-12 rounded-full border border-slate-200 bg-white text-slate-700 shadow-lg hover:bg-slate-50"
      >
        ↑
      </button>
    </main>
